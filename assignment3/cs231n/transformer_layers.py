...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        alpha=torch.matmul(query.permute(0,2,1,3),key.permute(0,2,3,1))
        if attn_mask!=None:
          alpha=alpha.masked_fill(attn_mask==0,float('-inf'))
        alpha_softmax=torch.softmax(alpha/np.sqrt(E//H),dim=3) #(N,H,S,T)
        alpha_softmax=self.attn_drop(alpha_softmax)
        output=torch.matmul(alpha_softmax,value.permute(0,2,1,3))
