...
def compute_saliency_maps(X, y, model):
    ...
    scores=model(X)
    scores=scores.gather(1, y.view(-1, 1)).squeeze() # we only choose the co rrect class score
    scores.backward(torch.ones_like(scores))  # backward() need initial gradient tensor
    saliency=X.grad.data
    saliency,_=torch.max(torch.abs(saliency),dim=1)  # torch.max return two tensors. one is the max value and the other is max index(we will not use that).
