...
def sample_noise(batch_size, dim, seed=None):
    ...
    tensor_random=torch.rand(batch_size, dim) * 2 - 1
...
def discriminator_loss(logits_real, logits_fake):
    ...
    loss_from_data = bce_loss(logits_real, real_labels.squeeze())
    # this "logits_fake" is equivalent to "logits_fake" from generator_loss()
    loss_from_sample = bce_loss(logits_fake, fake_labels.squeeze()) 
...
def build_dc_generator(noise_dim=NOISE_DIM):
    ...
    model=nn.Sequential(nn.Linear(noise_dim, 1024), # [batch_szie, 1024]
        nn.ReLU(),
